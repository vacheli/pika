...
func (s *Topom) tryFixReplicationRelationships(ctx *context, recoveredGroupServers []*redis.ReplicationState, masterLen int) {
	for _, state := range recoveredGroupServers {
		...
		err = s.tryFixReplicationRelationship(group, state.Server, state, masterLen)
		if err != nil {
...
func (s *Topom) tryFixReplicationRelationship(group *models.Group, groupServer *models.GroupServer, state *redis.ReplicationState, masterLen int) (err error) {
