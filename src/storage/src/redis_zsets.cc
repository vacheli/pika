...
Status RedisZSets::Open(const StorageOptions& storage_options, const std::string& db_path) {
  ...
  if (!storage_options.share_block_cache && storage_options.block_cache_size > 0) {
    ...
    data_cf_table_ops.block_cache = rocksdb::NewLRUCache(storage_options.block_cache_size,-1,true);
    score_cf_table_ops.block_cache = rocksdb::NewLRUCache(storage_options.block_cache_size,-1,true);
  }
